...
@section('title', 'List Why fenesta')

@section('content_header')

<div class="row">
<div class="col-md-8 text-left">
    <h1 class="m-0 text-dark">{{$title}} List </h1></div>
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/addSolutions/{{$type}}"><i class="fa fa-plus"></i> Add New</a>
...
<th><input type="checkbox" id="select_all" /></th>
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$fenestaWorlds->id}}" value="{{$fenestaWorlds->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{strip_tags($fenestaWorlds->title)}}</td>
			
				<td><img height="50px" width="50px" src={!!asset("images/solutions/$fenestaWorlds->mainimage")!!}></td>
                	<td>{{$fenestaWorlds->status}}</td>
				<td><a title="Edit" href="{{URL('admin/editSolutions',['id' => $fenestaWorlds->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$fenestaWorlds->id}}" title="Delete" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
