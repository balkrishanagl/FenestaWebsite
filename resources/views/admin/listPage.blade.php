...
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/addPage"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0"> <input type="hidden" class="tb" value="pages">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$page->id}}" value="{{$page->id}}"/></td>
				<td>{{$i}}</td>
				{{--<td>{{$page->meta_title}}</td>
				<td>{{$page->meta_description}}</td>--}}
				<td>{{$page->page_title}}</td>
				{{-- <td>{{$page->page_description}}</td> --}}
				<td><img  width="200px" src={!!asset("images/$page->banner_image")!!}></td>
				<td>{{$page->status}}</td>
				<td><a title="Edit {{$page->page_title}}" href="{{URL('admin/editPage',['id' => $page->id])}}"><i class="fas fa-edit"></i></a> <a href="#" title="Delete {{$page->page_title}}" data-id = "{{$page->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
