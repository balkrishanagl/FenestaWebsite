...
<div class="col-md-4 text-right">
              <a class="btn btn-primary"  title="Add New" href="{{ URL::to('/') }}/admin/faq"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			@foreach($faq as $faqs)
			<tr>
				<td><input type="checkbox" class="checkbox" data-emp-id="{{$faqs->id}}" value="{{$faqs->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$faqs->title}}</td>
				{{-- <td>{{$faqs->answer}}</td> --}}
				<td>{{date("d M Y",strtotime($faqs->created_at))}}</td>
                <td>{{$faqs->status}}</td>
				<td><a title="Edit" href="{{URL('admin/editFaq',['id' => $faqs->id])}}"><i class="fas fa-edit"></i></a> <a title="Delete" href="javascript:void(0)" data-id = "{{$faqs->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
