...
@section('title', 'Signature Studios List')
...
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{URL('admin/addshowroom')}}"><i class="fa fa-plus"></i> Add New</a> 
...
<div class="card-body table-responsive p-0"><input type="hidden" class="tb" value="showrooms">
	...
	<table class="data-table table table-head-fixed text-nowrap  table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$showrooms->id}}" value="{{$showrooms->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$showrooms->dealer_name}}</td>
				<td>{{$showrooms->city}}</td>
				<td>{{$showrooms->contact_person}}</td>
				<td>{{$showrooms->created_at}}</td>
				<td>{{$showrooms->status}}</td>
				<td><a title="Edit" href="{{URL('admin/editShowroom',['id' =>$showrooms->id])}}"><i class="fas fa-edit"></i></a> <a href="javascript:void(0)" data-id = "{{$showrooms->id}}" class="delete" title="Delete"><i class="fas fa-trash-alt"></i></a> </td>
