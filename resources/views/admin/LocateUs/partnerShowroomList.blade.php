...
@section('title', 'List Partner Showroom')
...
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{URL('admin/addParterShowroom')}}"><i class="fa fa-plus"></i> Add New</a> 
...
<div class="card-body table-responsive p-0"><input type="hidden" class="tb" value="partner_showrooms">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			<?php $i = 1; ?>
			@foreach($partnerShowroomList as $partnerShowroom)
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$partnerShowroom->id}}" value="{{$partnerShowroom->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$partnerShowroom->dealer_name}}</td>
				<td>{{$partnerShowroom->city}}</td>
				<td>{{$partnerShowroom->contact_person}}</td>
				<td>{{$partnerShowroom->created_at}}</td>
				<td>{{$partnerShowroom->status}}</td>
				<td><a title="Edit" href="{{URL('admin/editPartnerShowroom',['id' =>$partnerShowroom->id])}}"><i class="fas fa-edit"></i></a> <a  title="Delete"  href="javascript:void(0)" data-id = "{{$partnerShowroom->id}}" class="delete"><i class="fas fa-trash-alt"></i></a> </td>
