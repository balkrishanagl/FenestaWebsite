...
@section('title', ' Awards & Accreditations List')
...
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Award"><i class="fa fa-plus"></i> Add New</a>
...
<th>Status</th>
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$newz->id}}" value="{{$newz->id}}"/></td>
				<td>{{$i}}</td>
				<td>{!!$newz->description!!}</td>
				<td><img height="50px" width="50px" src={!!asset("images/awards/$newz->image")!!}></td>
                <td>{{ $newz->status }}</td>
				<td><a title="Edit" href="{{URL('admin/editAward',['id' => $newz->id])}}"><i class="fas fa-edit"></i></a> <a title="Delete" href="#" data-id = "{{$newz->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
