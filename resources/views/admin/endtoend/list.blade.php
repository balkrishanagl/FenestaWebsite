...
@if($type == 1)
@section('title', 'End to End Services')
@endif

@if($type == 2)
@section('title', 'Durable Safe Services')
@endif

@section('content_header')
<div class="row">
<div class="col-md-8 text-left">
    <h1 class="m-0 text-dark"> @if($type == 1) End to End  @endif  @if($type == 2) Durable Safe  @endif Services List </h1></div>
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/endtoend/{{ $type }}"><i class="fa fa-plus"></i> Add New</a>
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$banner->id}}" value="{{$banner->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{strip_tags($banner->title)}}</td>
				<td><img height="50px" width="50px" src={!!asset("images/services/$banner->image")!!}></td>
				<td>{{($banner->status)}}</td>
				<td><a title="Edit" href="{{URL('admin/endtoend/edit',['id' => $banner->id ] )}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$banner->id}}" title="Delete" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
...
<script>
	$(document).ready(function () {
		...
		$('#ok_button').click(function(){
  			$.ajax({
   				url:"{{url('/admin/endtoend/delete/')}}/"+banner_id,
   				beforeSend:function(){
