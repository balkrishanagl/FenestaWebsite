...
<div class="card-body table-responsive p-0"> <input type="hidden" class="tb" value="lp_leads">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr>
<td><input type="checkbox" class="checkbox" data-emp-id="{{$fenestaWorlds->id}}" value="{{$fenestaWorlds->id}}"/></td>

				<td>{{$i}}</td>
				<td>{{$fenestaWorlds->name}}</td>
				<td>{{$fenestaWorlds->email}}</td>
				<td>{{$fenestaWorlds->phone}}</td>
				<td>@if($fenestaWorlds->verifyOtp==0) Not Verified @else OTP Verified @endif</td>
				<td>{{$fenestaWorlds->city}}</td>
				<td>{{ date("d, M Y",strtotime($fenestaWorlds->cur_time)) }}</td>
                <td>{{$fenestaWorlds->utm_source}}</td>
                <td>{{$fenestaWorlds->device}}</td>
 ...
 {{--   @include('pagination.default', ['paginator' => $newsletters]) --}}
    <div class="page-div">
