...
@section('title', ' Trim Option List')
...
<div class="col-md-4 text-right">
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Trim"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0">  <input type="hidden" class="tb" value="trim_options">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$newz->id}}" value="{{$newz->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$newz->title}}</td>
				<td>
                    @if($newz->type=='Single')
                    <img height="50px" width="50px" src={!!asset("images/trim/$newz->image")!!}>
                    @else
                     @foreach(explode(',',$newz->image) as $mm)
                       <img height="50px" width="50px" src={!!asset("images/trim/$mm")!!}>
                      
                     @endforeach
                    @endif
                </td>
                <td>{{$newz->type}} Image</td>
                <td>{{$newz->status}} </td>
				<td><a title="Edit" href="{{URL('admin/editTrim',['id' => $newz->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$newz->id}}" title="Delete" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
