...
@section('title', 'Client Appreciation List')
...
<div class="col-md-4 text-right">
              <a class="btn btn-primary"  title="Add New" href="{{ URL::to('/') }}/admin/appreciation"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
            ...
            @endphp
			@foreach($appreciation as $appreciations)
			<tr>
              <td><input type="checkbox" class="checkbox" data-emp-id="{{$appreciations->id}}" value="{{$appreciations->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$appreciations->name}}</td>
				<td>{{$appreciations->city}}</td>
				<td><img height="50px" width="50px" src={!!asset("images/appreciation/$appreciations->image")!!}></td>
              <td>{{$category[$appreciations->category]}}</td>
                <td>{{$appreciations->status}}</td>
				<td><a title="Edit" href="{{URL('admin/editAppreciation',['id' => $appreciations->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$appreciations->id}}" title="Delete" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
