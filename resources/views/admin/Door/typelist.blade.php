...
@section('title', 'Door Product List')
...
<div class="col-md-4 text-right">
              <a class="btn btn-primary" title="Add New"  href="{{ URL::to('/') }}/admin/Door/typeadd"><i class="fa fa-plus"></i> Add New</a>
...
<!--				<th>Type</th>-->
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$banner->id}}" value="{{$banner->id}}"/></td>
				...
				<td>{{$banner->title}}</td>
				{{-- <td>{{$banner->type}}</td> --}}
				<td><img height="50px" width="50px" src={!!asset("images/windowdoortype/$banner->image")!!}></td>
				<td>{{$banner->status}}</td>
				<td><a title="Edit" href="{{URL('admin/Door/typeedit',['id' => $banner->id])}}"><i class="fas fa-edit"></i></a> &nbsp; <a href="{{URL('admin/Door/type',['id' => $banner->id])}}" title="Type"><i class="fa fa-window-restore" aria-hidden="true"></i></a> &nbsp;  <a href="{{URL('admin/Handlelock/list',['id' => $banner->id])}}" title="Handle & Lock"><i class="fa fa-lock" aria-hidden="true"></i></a> &nbsp;  <a href="#" title="Delete" data-id = "{{$banner->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
