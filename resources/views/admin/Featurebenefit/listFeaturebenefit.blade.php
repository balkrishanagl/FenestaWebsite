...
@section('title', ' Feature benefits List')
...
<div class="col-md-4 text-right">
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Featurebenefit"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$testimonial->id}}" value="{{$testimonial->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$testimonial->name}}</td>
				<td><img height="50px" width="50px" src={!!asset("images/featurebenefit/icon/$testimonial->icon")!!}></td>
<td>{{$testimonial->status}} </td>
				<td><a title="Edit" href="{{URL('admin/editFeaturebenefit',['id' => $testimonial->id])}}"><i class="fas fa-edit"></i></a> <a href="javascript:void(0)" data-id = "{{$testimonial->id}}" class="delete" title="Delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
