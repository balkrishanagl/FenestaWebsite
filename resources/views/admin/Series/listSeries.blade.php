...
<div class="col-md-4 text-right">
              <a class="btn btn-primary"  title="Add New" href="{{ URL::to('/') }}/admin/Series"><i class="fa fa-plus"></i> Add New</a>
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$newz->id}}" value="{{$newz->id}}"/></td>
				...
				<td>{{$newz->title}}</td>
				<td>
                    <img height="50px" width="50px" src="{{asset("images/series/$newz->image")}}" >
                   
                </td><td>{{$newz->status}} </td>
				<td><a  title="Edit" href="{{URL('admin/editSeries',['id' => $newz->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$newz->id}}" title="Delete" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
