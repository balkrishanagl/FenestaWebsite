...
@section('title', 'Color Option List')
...
<div class="col-md-4 text-right">
    @if($product_id!='')
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Coloroption/{{ $product_id }}"><i class="fa fa-plus"></i> Add New</a>
    @else
               <a class="btn btn-primary"  title="Add New"  href="{{ URL::to('/') }}/admin/Coloroption"><i class="fa fa-plus"></i> Add New</a>
    @endif
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr> <td><input type="checkbox" class="checkbox" data-emp-id="{{$banner->id}}" value="{{$banner->id}}"/></td>
				...
				<td>{{$banner->wtitle}}</td>
				<td>{{$banner->type}}</td>
				<td><img height="50px" width="50px" src={!!asset("images/coloroption/$banner->image")!!}></td>  <td>{{$banner->status}} </td>
				
				<td><a title="Edit" href="{{URL('admin/Coloroption/edit',['id' => $banner->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$banner->id}}" class="delete" title="Delete"><i class="fas fa-trash-alt"></i></a></td> 
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
