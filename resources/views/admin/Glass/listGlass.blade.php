...
@section('title', ' Glass Option List')
...
<div class="col-md-4 text-right">
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Glass"><i class="fa fa-plus"></i> Add New</a>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$newz->id}}" value="{{$newz->id}}"/></td>
				...
				<td>
                     ...
                     @foreach(explode(',',$newz->image) as $mm)
                       <img height="50px" width="50px" src={!!asset("images/glass/$mm")!!}>
                      
                     @endforeach
                </td>
                <td>{{$newz->upload_type}} </td>
                <td>{{$newz->status}} </td>
				<td><a title="Edit" href="{{URL('admin/editGlass',['id' => $newz->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$newz->id}}" title="Delete" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
