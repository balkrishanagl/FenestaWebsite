...
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/addAbout/{{$type}}"><i class="fa fa-plus"></i> Add New</a>
...
<th>Status</th>
		...
		<tbody>
			<?php $i = 1; ?>
			@foreach($listings as $newz)
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$newz->id}}" value="{{$newz->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$newz->title}}</td>
                <td> @if($newz->image) <img src="{{asset("images/about/$newz->image")}}" alt="No Image" width="70px" > @else No Image @endif </td>
<td>{{ $newz->status }}</td>
				<td><a title="Edit" href="{{URL('admin/editAbout',['id' => $newz->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$newz->id}}" class="delete" title="Delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
