...
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/fenestaWorld"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0"><input type="hidden" class="tb" value="gallery_images">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			<?php $i = 1; ?>
			@foreach($fenestaWorld as $fenestaWorlds)
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$fenestaWorlds->id}}" value="{{$fenestaWorlds->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$fenestaWorlds->title}}</td>
				<td><img height="50px" width="50px" src={!!asset("images/fenesta_world/$fenestaWorlds->image")!!}></td>
                <td>{{$fenestaWorlds->status}}</td>
				<td><a title="Edit" href="{{URL('admin/editFenestaWorld',['id' => $fenestaWorlds->id])}}"><i class="fas fa-edit"></i></a> <a title="Delete" href="javascript:void(0)" data-id = "{{$fenestaWorlds->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
