...
<div class="col-md-4 text-right">
              <a class="btn btn-primary" title="Add New"  href="{{ URL::to('/') }}/admin/Banner"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0"><input type="hidden" class="tb" value="home_banner_images">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr>
                <td><input type="checkbox" class="checkbox" data-emp-id="{{$banner->id}}" value="{{$banner->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{strip_tags($banner->heading)}}</td>
				<td>{{strip_tags($banner->sub_heading)}}</td>
				<td><img width="100px" src={!!asset("images/home_banner/big/$banner->home_banner_image")!!}></td>
                <td>{{ $banner->status }}</td>
				<td><a title="Edit" href="{{URL('admin/editBanner',['id' => $banner->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$banner->id}}" class="delete" title="Delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
