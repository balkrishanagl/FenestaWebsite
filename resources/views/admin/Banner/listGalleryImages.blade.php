...
<div class="col-md-4 text-right">
    <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/galleryImages"><i class="fa fa-plus"></i> Add New</a>
...
<div class="card-body table-responsive p-0">
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$galleryData->id}}" value="{{$galleryData->id}}"/></td>
				<td>{{$i}}</td>
				<td>{!! $galleryData->heading !!}</td>
				<td><img  width="100px" src={!!asset("images/gallery_images/$galleryData->image")!!}></td>
                <td>{{ $galleryData->status }}</td>
              {{--  <td>@if($galleryData->showon==1) Home @elseif($galleryData->showon==2) Gallery @else Both Page @endif </td>--}}
				<td><a title="Edit" href="{{URL('admin/editGalleryImage',['id' => $galleryData->id])}}"><i class="fas fa-edit"></i></a> <a title="Delete" href="#" data-id = "{{$galleryData->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
