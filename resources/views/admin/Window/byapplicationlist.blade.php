...
@section('title', 'Window By Application')
...
<div class="col-md-4 text-right">
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Window/byapplicationadd"><i class="fa fa-plus"></i> Add New</a>
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr> <td><input type="checkbox" class="checkbox" data-emp-id="{{$banner->id}}" value="{{$banner->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$banner->title}}</td><td>{{$banner->status}}</td>
				<td><a title="Edit" href="{{URL('admin/Window/byapplicationedit',['id' => $banner->id])}}"><i class="fas fa-edit"></i></a> <a href="#" data-id = "{{$banner->id}}" title="Delete" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
