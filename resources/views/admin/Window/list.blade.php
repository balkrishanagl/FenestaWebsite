...
<div class="col-md-8 text-left">
    <h1 class="m-0 text-dark">List of @if(!empty($s_title)) {{ $s_title }} @endif  Type </h1></div>
<div class="col-md-4 text-right">
      @if($product_id !='' )
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Window/add/{{ $product_id }}"><i class="fa fa-plus"></i> Add New</a>
    @else
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Window/add"><i class="fa fa-plus"></i> Add New</a>
    @endif
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr> <td><input type="checkbox" class="checkbox" data-emp-id="{{$banner->id}}" value="{{$banner->id}}"/></td>
				...
				<td>{{$banner->title}}</td>
				<td>{{ $windowt->title }}</td>
				<td><img height="50px" width="50px" src={!!asset("images/windowtype/$banner->image")!!}></td><td>{{$banner->status}}</td>
				<td><a title="Edit" href="{{URL('admin/Window/edit',['id' => $banner->id])}}"><i class="fas fa-edit"></i></a> <a title="Delete" href="#" data-id = "{{$banner->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
