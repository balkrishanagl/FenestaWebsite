...
<div class="col-md-4 text-right">
              <a class="btn btn-primary" title="Add New" href="{{ URL::to('/') }}/admin/Brochure"><i class="fa fa-plus"></i> Add New</a>
	...
	<table class="data-table table table-head-fixed text-nowrap table-bordered" id="user_table">
		...
		<tbody>
			...
			<tr><td><input type="checkbox" class="checkbox" data-emp-id="{{$banner->id}}" value="{{$banner->id}}"/></td>
				<td>{{$i}}</td>
				<td>{{$banner->title}}</td>
				<td><img  width="100px" src={!!asset("images/brochure/$banner->image")!!}></td>
				<td>{{$banner->status}}</td>
				<td><a title="Edit" href="{{URL('admin/Brochureedit',['id' => $banner->id])}}"><i class="fas fa-edit"></i></a> <a title="Delete" href="#" data-id = "{{$banner->id}}" class="delete"><i class="fas fa-trash-alt"></i></a></td>
			</tr>
			<?php $i++; ?>
			@endforeach
		</tbody>
