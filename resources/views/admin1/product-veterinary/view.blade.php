      ...
      <div class="row">
        ...
        <form class="form-horizontal" method="POST" action="<?php echo url(ADMIN_FOLDER);?>/<?php echo $page_name;?>/save" enctype="multipart/form-data">
             ...
             <input type="hidden" required="" name="id" value="{{ (isset($data_row->id))?$data_row->id:'' }}">
              ...
              <div class="box-body">
                 ...
                 <div class="form-group">
                  ...
                  <select name="category" class="form-control select" required  >          
                        @foreach ( Config::get('constants.CONS_VETERINARY_CATEGORY_ARRAY')  as $key =>$val)
                            <?php 
                              $selected='';
                              if(isset($data_row['category']) && $data_row['category']==$key){
                                $selected='selected'; 
                              }elseif(old('category')==$key){
                                 $selected='selected';
                              } 
                            ?>
              ...
              <?php /*
            <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label ">Therapeutic Area</label>        
                  <div class="col-sm-10">                   
                <select name="category_id" class="form-control select select2">
        
                           <option value="0"></option>
                           <?php foreach ($categorys as $key => $category) { ?>
                             <option value="<?php echo $category->id;?>" <?php echo (isset($data_row['category_id']) && $data_row['category_id']==$category->id)?'selected':'';?>><?php echo $category->cat_name;?></option>
        ...
        */?>
               <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <input type="text" required="" class="form-control" id="brand_name" name="brand_name" value="{{ (isset($data_row->brand_name))?$data_row->brand_name:old('brand_name') }}">
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <textarea required="" class="form-control"  id="ingredient" name="ingredient" >{{ (isset($data_row->ingredient))?$data_row->ingredient:old('ingredient') }}</textarea>
