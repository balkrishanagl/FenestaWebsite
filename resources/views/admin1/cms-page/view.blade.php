      ...
      <div class="row">
        ...
        <form class="form-horizontal" method="POST" action="<?php echo url(ADMIN_FOLDER);?>/<?php echo $page_name;?>/save" enctype="multipart/form-data">
             ...
             <?php /*   <div class="form-group">
                 ...
                 <!--  <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Content4</label>

                  <div class="col-sm-10">
                  <textarea class="form-control summernote" id="summernote" name="content4" >{{ (isset($data_row->content4))?$data_row->content4:old('content4') }}</textarea>       
         ...
         <p class="help-block">Image Size :1720 X 550 </p>
          ...
          <?php if(isset($data_row['image_mobile']) && $data_row['image_mobile']!=''){?>
                    <img src="{{ URL::asset('/') }}<?php echo $data_row['image_mobile'];?>" height="100px">
               ...
               <div class="form-group">
                ...
                <option value="{{ $key }}"   {{ $selected }}  >
                 {{ $val }}
                  ...
                  <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Page Layout(Enter only when need new layout)</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="layout" name="layout" value="{{ (isset($data_row->layout))?$data_row->layout:old('layout') }}">
                  </div>
                </div>
         
              <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Menu Parent Page</label>        
                  <div class="col-sm-10">                   
                <select name="parent_menu_page_id" class="form-control select">
        
                           <option value="0"></option>
                           <?php foreach ($cmspages as $cmspage) { ?>
                             <option value="<?php echo $cmspage->id;?>" <?php echo (isset($data_row['parent_menu_page_id']) && $data_row['parent_menu_page_id']==$cmspage->id)?'selected':'';?>><?php echo $cmspage->name;?>(<?php echo $cmspage->slug;?>)
