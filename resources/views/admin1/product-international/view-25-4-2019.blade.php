             ...
             <input type="hidden" required="" name="id" value="{{ (isset($data_row->id))?$data_row->id:'' }}">
              ...
              <div class="box-body">
                 ...
                 <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Main Category</label>        
                  <div class="col-sm-10">                       
                  <select name="maincategory_id" class="form-control select" required  >          
                        @foreach ( Config::get('constants.CONS_MAINCATEGORY_ARRAY')  as $key =>$val)
                            <?php 
                              $selected='';
                              if(isset($data_row['maincategory_id']) && $data_row['maincategory_id']==$key){
                                $selected='selected'; 
                              }elseif(old('maincategory_id')==$key){
                                 $selected='selected';
                              } 
                            ?>
