...
class BlogTagCtrlAdmin extends Controller {
...
public function __construct()
  ...
  public function index() {
    ...
    if(isset($_GET['delete'])){
        $delete_status = BlogTagModel::find($_GET['delete'])->delete();
 ...
 $data_rows=BlogTagModel::orderBy('id', 'desc');
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
    ...
    $data = BlogTagModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
                    'title' => 'View Blog Tag',                    
                    'page_name' => $this->name_url_folder,                     
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = BlogTagModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
        ...
        if($id){            
          $array_validate['slug']="required|unique:blog_tag,slug,$id"; 
        } else {
          $array_validate['slug'] = "required|unique:blog_tag|max:255";  
        } 
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=BlogTagModel::create($_POST);

                    } else {
                               
            
                $save=BlogTagModel::find($id);              
                $save->fill($_POST);
