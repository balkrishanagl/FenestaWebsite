...
class ProductApiCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'product-api';
    }
...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
      ...
      if($_GET['active_ingredient']){       
       $data_rows->where('active_ingredient', 'LIKE',"%{$_GET['active_ingredient']}%" );
      }
        if($_GET['cas_no']){       
       $data_rows->where('cas_no', 'LIKE',"%{$_GET['cas_no']}%" );
      }
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = ProductApiModel::find($_GET['delete'])->delete();
 ...
 $data_rows=ProductApiModel::orderBy('id', 'desc')->where('is_delete',1);
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
    ...
    public function view($id) {
      ...
      $this->roleChecking(); 
    $data = ProductApiModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
                    'title' => 'View',                    
                    'page_name' => $this->name_url_folder,                     
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = ProductApiModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
        ...
        $array_validate['active_ingredient']="required";       
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=ProductApiModel::create($_POST);

                    } else {
                               
            
                $save=ProductApiModel::find($id);              
                $save->fill($_POST);
        ...
        $data_rows=ProductApiModel::orderBy('id', 'desc');
        $data_rows=$this->getFilter($data_rows);
        ...
        if(count($data_rows)>0){
            ...
            $export_data="ID,active_ingredient,cas_no,status,CreatedAt,UpdatedAt \n";
            foreach($data_rows as $value){
                 $export_data.='"'.$value->id.'",'; 

                $export_data.='"'.$value->active_ingredient.'",';           
                $export_data.='"'.$value->cas_no.'",';            
