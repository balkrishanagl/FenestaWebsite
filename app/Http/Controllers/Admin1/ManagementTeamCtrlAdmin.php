...
use App\ManagementTeamModel;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
class ManagementTeamCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'management-team';
    }
...
public function roleChecking() {
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = ManagementTeamModel::find($_GET['delete'])->delete();
 ...
 $data_rows=ManagementTeamModel::orderBy('id', 'desc');
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
    ...
    public function view($id) {
      ...
      $this->roleChecking(); 
    $data = ManagementTeamModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
                    'title' => 'View member',                    
                    'page_name' => $this->name_url_folder,                     
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = ManagementTeamModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
    ...
    $this->validate($request,$array_validate);  
        ...
        $new_file_name=$filename_remove[0];       
            ...
            $destinationPath = public_path('/media/images/management_team/image');
            $image->move($destinationPath, $image_name);
           $_POST['image']='media/images/management_team/image/'.$image_name;
        } else {
            if(isset($_POST['image_delete'])){        
              $_POST['image']='';
               ...
               if($id==''){           
                      $save=ManagementTeamModel::create($_POST);

                    } else {
                               
            
                $save=ManagementTeamModel::find($id);              
                $save->fill($_POST);
