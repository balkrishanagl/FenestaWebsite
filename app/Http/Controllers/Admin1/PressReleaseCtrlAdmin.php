...
use App\PressReleaseModel;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
class PressReleaseCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'press-release';
    }
...
public function roleChecking() {
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = PressReleaseModel::find($_GET['delete'])->delete();
 ...
 $data_rows=PressReleaseModel::orderBy('sorting', 'desc');
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
    ...
    public function view($id) {
      ...
      $this->roleChecking(); 
    $data = PressReleaseModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
                    'title' => 'View',                    
                    'page_name' => $this->name_url_folder,                     
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = PressReleaseModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
    ...
    $this->validate($request,$array_validate);  
        ...
        $new_file_name=$filename_remove[0];       
            ...
            $destinationPath = public_path('/media/images/mediagallery/image_path');
            $image_path->move($destinationPath, $image_name);
           $_POST['image_path']='media/images/mediagallery/image_path/'.$image_name;
        } else {
          ...
          //$_POST['photos']=implode(',',$_POST['photos']);
               if($id==''){           
                      $save=PressReleaseModel::create($_POST);

                    } else {
                               
            
                $save=PressReleaseModel::find($id);              
                $save->fill($_POST);
        ...
        $data_rows=PressReleaseModel::orderBy('id', 'desc');
        $data_rows=$this->getFilter($data_rows);
