...
class CollaborationCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'collaboration';
    }
...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
      ...
      if($_GET['company_name']){       
       $data_rows->where('company_name', 'LIKE',"%{$_GET['company_name']}%" );
      }
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = CollaborationModel::find($_GET['delete'])->delete();
 ...
 $data_rows=CollaborationModel::orderBy('id', 'desc');
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
    ...
    public function view($id) {
      ...
      $this->roleChecking(); 
    $data = CollaborationModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
                    'title' => 'View',                    
                    'page_name' => $this->name_url_folder,                     
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = CollaborationModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
        ...
        $array_validate['company_name']="required"; 
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=CollaborationModel::create($_POST);

                    } else {
                               
            
                $save=CollaborationModel::find($id);              
                $save->fill($_POST);
      ...
      $this->roleChecking(); 
        ...
        $tot_record_found=0;
           if(count($data_rows)>0){
            ...
            $export_data="Id,Company name,Email,Country,Department,ContactPerson,Telephone,Website,Subject,BriefDescription,Status,IP,Date,UpdatedAt \n";
            foreach($data_rows as $value){
          $export_data.='"'.$value->id.'",';        
         $export_data.='"'.$value->company_name.'",';       
         $export_data.='"'.$value->email.'",';       
         $export_data.='"'.$value->country.'",';       
         $export_data.='"'.$value->department.'",';       
         $export_data.='"'.$value->contact_person.'",';       
         $export_data.='"'.$value->telephone.'",';       
         $export_data.='"'.$value->website.'",';        
         $export_data.='"'.$value->subject.'",';             
         $export_data.='"'.$value->brief_description.'",';        
