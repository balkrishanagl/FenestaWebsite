...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
      ...
      if($_GET['from_url']){       
       $data_rows->where('from_url', 'LIKE',"%{$_GET['from_url']}%" );
      }

if($_GET['to_url']){       
       $data_rows->where('to_url', 'LIKE',"%{$_GET['to_url']}%" );
      }
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = RedirectUrlModel::find($_GET['delete'])->delete();
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=RedirectUrlModel::create($_POST);
        ...
        if(count($data_rows)>0){
            ...
            $export_data="ID,From Url,To Url,CreatedAt,UpdatedAt \n";
