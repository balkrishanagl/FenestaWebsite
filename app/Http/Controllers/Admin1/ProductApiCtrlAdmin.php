...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
      ...
      if($_GET['product']){       
       $data_rows->where('product', 'LIKE',"%{$_GET['product']}%" );
      }
        if($_GET['therapeutic']){       
       $data_rows->where('therapeutic', 'LIKE',"%{$_GET['therapeutic']}%" );
      }
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        ...
        $array_validate['product']="required";       
        $array_validate['therapeutic']="required";       
    ...
    $this->validate($request,$array_validate);  
        ...
        if(count($data_rows)>0){
            ...
            $export_data="ID,product,therapeutic,status,CreatedAt,UpdatedAt \n";
            foreach($data_rows as $value){
                 $export_data.='"'.$value->id.'",'; 

                $export_data.='"'.$value->product.'",';           
                $export_data.='"'.$value->therapeutic.'",';            
