...
class BlogCommentCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'blog-comment';
    }
...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
     ...
     if($_GET['blog_name']){       
       $data_rows->where('blog_name', 'LIKE',"%{$_GET['blog_name']}%" );
      } 
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = BlogCommentModel::find($_GET['delete'])->delete();
 ...
 $data_rows=BlogCommentModel::orderBy('id', 'desc');
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
    ...
    public function view($id) {
      ...
      $this->roleChecking(); 
    $data = BlogCommentModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
                    'title' => 'View',                    
                    'page_name' => $this->name_url_folder,                     
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = BlogCommentModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=BlogCommentModel::create($_POST);

                    } else {
                               
            
                $save=BlogCommentModel::find($id);              
                $save->fill($_POST);
      ...
      $this->roleChecking(); 
        ...
        $tot_record_found=0;
           if(count($data_rows)>0){
            ...
            $export_data="Id,Blog Name,Name,Email,Message,Status,IP,CreatedAt,UpdatedAt \n";
            foreach($data_rows as $value){
          $export_data.='"'.$value->id.'",';        
         $export_data.='"'.$value->blog_name.'",';       
