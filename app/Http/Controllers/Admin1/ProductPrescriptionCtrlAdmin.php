...
use App\ProductPrescriptionModel;
use App\ProductCategoryModel;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
class ProductPrescriptionCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'product-prescription';
    }
...
public function roleChecking() {
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = ProductPrescriptionModel::find($_GET['delete'])->delete();
...
$categorys=ProductCategoryModel::orderBy('cat_name', 'asc')->get();
 ...
 $data_rows=ProductPrescriptionModel::orderBy('id', 'desc')->where('is_delete',1);
 $data_rows=$this->getFilter($data_rows); 
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);

  
    $page_array=array(                    
                    'title' => 'Products Prescription',                    
    ...
    public function view($id) {
      ...
      $this->roleChecking();  
    $data = ProductPrescriptionModel::where('id', $id)->first();
   $categorys=ProductCategoryModel::orderBy('cat_name', 'asc')->get();
            if($data){
                $page_array=array(                    
                    'title' => 'View',                    
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = ProductPrescriptionModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=ProductPrescriptionModel::create($_POST);

                    } else {
                               
            
                $save=ProductPrescriptionModel::find($id);              
                $save->fill($_POST);
