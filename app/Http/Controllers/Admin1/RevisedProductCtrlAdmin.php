...
use App\RevisedProductModel;
use App\RevisedListModel;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
class RevisedProductCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'revised-product';
    }
...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
      ...
      if($_GET['list_id']){       
       $data_rows->where('list_id', 'LIKE',"%{$_GET['list_id']}%" );
      }
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = RevisedProductModel::find($_GET['delete'])->delete();
...
$lists=RevisedListModel::orderBy('name', 'asc')->get();


 $data_rows=RevisedProductModel::orderBy('id', 'desc');
 $data_rows=$this->getFilter($data_rows); 
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);

  
    $page_array=array(                    
                    'title' => 'Revised Products',                    
    ...
    public function view($id) {
      ...
      $this->roleChecking();  
    $data = RevisedProductModel::where('id', $id)->first();
   $lists=RevisedListModel::orderBy('name', 'asc')->get();
            if($data){
                $page_array=array(                    
                    'title' => 'View',                    
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = RevisedProductModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
         ...
         $array_validate=array();
          $array_validate['list_id']="required";     
        $array_validate['name']="required";       
        $array_validate['pack_size']="required";       
        $array_validate['old_mrp']="required";       
        $array_validate['revised_mrp']="required";       
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=RevisedProductModel::create($_POST);

                    } else {
                               
            
                $save=RevisedProductModel::find($id);              
                $save->fill($_POST);
  ...
  public function export(Request $request){  
        ...
        if(count($data_rows)>0){
            ...
            $export_data="ID,Name,Pack Size,Old MRP,Revised MRP,Status,CreatedAt,UpdatedAt \n";
            foreach($data_rows as $value){
                 $export_data.='"'.$value->id.'",'; 

                       
                $export_data.='"'.$value->name.'",';           
                $export_data.='"'.$value->pack_size.'",';           
                $export_data.='"'.$value->old_mrp.'",';          
                $export_data.='"'.$value->revised_mrp.'",'; 
