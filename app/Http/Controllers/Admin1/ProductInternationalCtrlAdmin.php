...
use App\ProductInternationalModel;
use App\ProductCategoryModel;
use App\CountriesModel;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
class ProductInternationalCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'product-international';
    }
...
public function roleChecking() {
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = ProductInternationalModel::find($_GET['delete'])->delete();
...
$countries=CountriesModel::orderBy('name', 'asc')->get();


 $data_rows=ProductInternationalModel::orderBy('id', 'desc');
 $data_rows=$this->getFilter($data_rows); 
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);

  
    $page_array=array(                    
                    'title' => 'Products International',                    
    ...
    public function view($id) {
      ...
      $this->roleChecking();  
    $data = ProductInternationalModel::where('id', $id)->first();
    $countries=CountriesModel::orderBy('name', 'asc')->get();
            if($data){
                $page_array=array(                    
                    'title' => 'View',                    
    ...
    public function add() {
      // $categorys=ProductCategoryModel::where('parent_id',8)->orderBy('cat_name', 'asc')->get();
        $countries=CountriesModel::orderBy('name', 'asc')->get();
             $data=array();
                $page_array=array(                    
                     'title' =>'Add',                    
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = ProductInternationalModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
...
*/
               ...
               if($id==''){           
                      $save=ProductInternationalModel::create($_POST);

                    } else {
                               
            
                $save=ProductInternationalModel::find($id);              
                $save->fill($_POST);
  ...
  public function export(Request $request){  
        ...
        if(count($data_rows)>0){
            ...
            $export_data="ID,Form name,Ingredient,Packdetails,Status,CreatedAt,UpdatedAt \n";
