...
class NewsletterCtrlAdmin extends Controller {
...
public function __construct()
    {
        ...
        $this->user =  \Auth::user();
        $this->name_url_folder= 'newsletter';
    }
...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
       ...
       if($_GET['email']){       
       $data_rows->where('email', 'LIKE',"%{$_GET['email']}%" );
      }
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = NewsletterModel::find($_GET['delete'])->delete();
 ...
 $data_rows=NewsletterModel::orderBy('id', 'desc');
 ...
 $data_rows= $data_rows->paginate(PAGINATE_LIMIT);
    ...
    public function view($id) {
      ...
      $this->roleChecking(); 
    $data = NewsletterModel::where('id', $id)->first();
            if($data){
                $page_array=array(                    
   ...
   if($_POST['submit_type']=='3' && $id!=''){
        $delete_status = NewsletterModel::find($id)->delete();
         $message_type="message_susuccess";
         $message_text="Record has been deleted successfully!";
         return redirect(ADMIN_FOLDER.'/'.$this->name_url_folder)->with($message_type,$message_text);
         ...
         if($id){            
          $array_validate['email']="required|unique:newsletter,email,$id"; 
        } else {
          $array_validate['email'] = "required|unique:newsletter|max:255";  
        } 
    ...
    $this->validate($request,$array_validate);  
            ...
            try
              { 
               ...
               if($id==''){           
                      $save=NewsletterModel::create($_POST);

                    } else {
                               
            
                $save=NewsletterModel::find($id);              
                $save->fill($_POST);
        ...
        if(count($data_rows)>0){
            ...
            $export_data="ID,Email,CreatedAt,UpdatedAt \n";
