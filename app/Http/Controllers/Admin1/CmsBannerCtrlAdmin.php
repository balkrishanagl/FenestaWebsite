...
public function roleChecking() {
  ...
  $admin_modules_array=explode(',',Auth::user()->admin_modules);
    ...
    if(isset($_GET['saerch']) && $_GET['saerch']=='field'){   
     ...
     if($_GET['type']){       
       $data_rows->where('type',$_GET['type']);
      }
  ...
  $this->roleChecking(); 
    ...
    if(isset($_GET['delete'])){
        $delete_status = CmsBannerModel::find($_GET['delete'])->delete();
   ...
   if($_POST['submit_type']=='3' && $id!=''){
    ...
    $this->validate($request,$array_validate);  
        ...
        $new_file_name=$filename_remove[0];       
            ...
            $destinationPath = public_path('/media/images/cms-banner/image_path');
            $image_path->move($destinationPath, $image_name);
           $_POST['image_path']='media/images/cms-banner/image_path/'.$image_name;
        } else {
        ...
        $new_file_name=$filename_remove[0];       
            ...
            $destinationPath = public_path('/media/file/cms-banner/download_file');
            $downloads->move($destinationPath, $downloads_name);
           $_POST['download_file']='media/file/cms-banner/download_file/'.$downloads_name;
        } else {
          ...
          //$_POST['photos']=implode(',',$_POST['photos']);
               if($id==''){           
                      $save=CmsBannerModel::create($_POST);
